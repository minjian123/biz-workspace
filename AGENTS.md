...
- **推送前先本地预检**：含代码 / 脚本 / CI 改动时，推送前先跑 `python3 bms/scripts/tools/preflight/check-preflight.py`（在 bms 仓根；`--fast` 秒级只跑 CI 配置 / 静态 / 基座与边界），全绿再推——避免「改一点、推一次、等整条流水线」的慢循环；CI 专属语义（如 job 里的 pytest 选项）已由预检在本地校验。
- **CI job 定义改动不在同一条流水线自验**：`.gitlab-ci.yml` 改动只负责创建流水线，job 级触发已不再监听 `.gitlab-ci.yml`（改按各域路径放行，纯 CI / 部署 / 后端改动不再重建前端）。因此改了某个 job 的定义后，它不会在这条推送里被跑到，要等下次命中对应路径；配置改动风险低、可回退，必要时手动跑一次 web 流水线即可验证。
